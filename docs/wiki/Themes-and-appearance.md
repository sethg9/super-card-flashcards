...
Choose an image, adjust dimming, or replace/remove it. SuperCard copies the image into managed storage, so moving its original does not remove your background. Images are centered and scaled to cover the background without stretching. Content panels remain opaque for readability.
...
PNG, JPEG, GIF, and WebP files are supported, up to 200 MB (209,715,200 bytes) and 40 megapixels. SVG is unsupported.
